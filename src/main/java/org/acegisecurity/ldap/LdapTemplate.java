...
public class LdapTemplate {
    ...
    public static final Logger LOGGER = Logger.getLogger(LdapTemplate.class.getName());
...
/**
    ...
    public boolean compare(final String dn, final String attributeName, final Object value) {
        ...
        class LdapCompareCallback implements LdapCallback {
            public Object doInDirContext(DirContext ctx)
                ...
                NamingEnumeration results = ctx.search(new LdapName(relativeName), comparisonFilter, new Object[] {value}, ctls);
    ...
    public boolean nameExists(final String dn) {
        Boolean exists = (Boolean) execute(new LdapCallback() {
                public Object doInDirContext(DirContext ctx)
                    throws NamingException {
                    try {
                        Object obj = ctx.lookup(new LdapName(LdapUtils.getRelativeName(dn, ctx)));
    ...
    public Object retrieveEntry(final String dn, final LdapEntryMapper mapper, final String[] attributesToRetrieve) {
        return execute(new LdapCallback() {
                public Object doInDirContext(DirContext ctx)
                    throws NamingException {
                    LOGGER.log(Level.FINE, "doInDirContext");
                    LOGGER.log(Level.FINE, ">>> LdapUtils.getRelativeName for DN \"{0}\" in getRelativeName = \"{1}\"", new String[]{ dn,String.valueOf(new LdapName(LdapUtils.getRelativeName(dn, ctx)))});
                    LOGGER.log(Level.FINE, ">>> attributesToRetrieve = {0}", joiner(attributesToRetrieve));
                    Attributes attbs = ctx.getAttributes(new LdapName(LdapUtils.getRelativeName(dn, ctx)), attributesToRetrieve);
                    Object output = mapper.mapAttributes(dn,attbs);
                    LOGGER.log(Level.FINE, ">>> mapper.mapAttributes {0} = {1}", new String[]{joiner(attributesToRetrieve) ,String.valueOf(output)});
                    return output;
                }
            });
    }

    private String joiner( String[] stringArray) {
        if (stringArray == null) {
            return "";
        }
    ...
    public Object searchForSingleEntry(final String base, final String filter, final Object[] params,
        ...
        return execute(new LdapCallback() {
                public Object doInDirContext(DirContext ctx)
                    ...
                    LOGGER.log(Level.FINE, ">>> Le DN avant modif = {0}", dn.toString());
                    LOGGER.log(Level.FINE, ">>> Le DN apres modif = {0}", encoderSpecialChar(dn.toString().replace("\"","")));
                    return mapper.mapAttributes(encoderSpecialChar(dn.toString().replace("\"","")), searchResult.getAttributes());
                    //return mapper.mapAttributes(dn.toString(), searchResult.getAttributes());
                }
            });
    }

    private static String encoderSpecialChar(final String inStr) {
        return inStr.replaceAll("\\\\,","\\\\2C");
        //.replaceAll("\\\\" ,"\\\\5c");
               /* .replaceAll(",","\\\\2c")
                .replaceAll("\"","\\\\22")
                .replaceAll("\\+","\\\\2b")
